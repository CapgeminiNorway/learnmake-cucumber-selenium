...
public class SharedContextSteps {
    ...
    private SharedContextSteps contextSteps;
    private String withBrowser;// = "chrome";

    // PicoContainer injects class ContextSteps
    public MyStepDefs(SharedContextSteps contextSteps) {
    ...
    private final String GITHUB_TOKEN = "e6f6d569e4c2b136b3e30b0e641f8b591da74acc"; // see known issues on https://github.com/bonigarcia/webdrivermanager

    // FIXME: make use of PROXY for internal usage
    private static final String PROXY_HOST =
            isLocalEnv() ?
                    "localproxy-url"
                    : "intranet-proxy-url";
    private static final int PROXY_PORT = 8088;
    private static final String NON_PROXY_HOSTS = "localhost, 127.0.0.1";
    ...
    public void setUp() throws Exception {
        ...
        if (isLocalEnv()) {
            // -DisLocal=true -DbaseUrl=https://www.BASE_URL.no
            ...
            if (!System.getProperties().containsKey("webdriver.gecko.driver")) {
                System.setProperty("webdriver.gecko.driver", "wdm_local/geckodriver");
            }
            if (!System.getProperties().containsKey("webdriver.chrome.driver")) {
                System.setProperty("webdriver.chrome.driver", "wdm_local/chromedriver");
            }
        }
        else {
        ...
        //webDrivers.put(Browser.chrome, initBrowserDriver(Browser.chrome));
    ...
    private RemoteWebDriver initBrowserDriver(Browser browser) {
        ...
        try {
            ...
            try {
                System.out.println(SharedContextSteps.class.getSimpleName()+"|" + "UnhandledAlertException: " + ex);
                Alert alert = browserDriver.switchTo().alert();
                System.out.println(SharedContextSteps.class.getSimpleName()+"|" + "alert.getText: " + alert.getText());
    ...
    public RemoteWebDriver getDriver(String browser) {
        if (StringUtils.isNotEmpty(browser)) {
            //Browser browser = Browser.valueOf(which);
    ...
    /*
    public RemoteWebDriver prepareWebDriver() { // with default values
        return prepareWebDriver( "", getLoginUrl() );
    }
    public RemoteWebDriver prepareWebDriver(String browser, String theUrl) {
    ...
    */
    ...
    public static boolean switchToTab(WebDriver driver, String tabName){
        System.out.println("Switch to " + tabName);
